# Principal Studio — local vs. remote modes

Design doc for splitting the standalone Electrobun principal studio (`@principal-ai/principal-studio`) into two operating modes so it can be used by people who **do** have the repo cloned and by people who **don't**, without forcing them to install the full Principal ADE desktop app.

Companion to:
- [`TRAIL_REPO_PURL_ROLLOUT.md`](https://github.com/principal-ai/industry-themed-file-city-panels) — phased adoption of `TrailRepo.id: Purl`. This doc assumes Phase 2 is done (CLI + principal-studio surfaces in this repo are both already converted).
- The CLI `trail` command at [`packages/cli/src/commands/trail.ts`](../packages/cli/src/commands/trail.ts).
- The viewer host at [`packages/principal-studio/src/bun/index.ts`](../packages/principal-studio/src/bun/index.ts) and renderer at [`packages/principal-studio/src/mainview/index.tsx`](../packages/principal-studio/src/mainview/index.tsx).
...
| 3 | `packages/principal-studio/src/bun/index.ts` | Add a `mode` discriminator. Today `loaded` is always `{ ok, payload, path }`; replace with `{ mode: 'local' | 'remote', payload, ... }`. Source-of-truth for mode: env (`TRAIL_MODE`) or inferred from absence of `TRAIL_REPO_ROOT`. |
| 4 | `packages/principal-studio/src/bun/index.ts` (`readFile` RPC) | Two implementations behind the discriminator. Local: today's `resolveSandboxed` + `fs.readFile`. Remote: build the `raw.githubusercontent.com/<owner>/<repo>/<sha>/<path>` URL from `repos[i]` (or `authoredAt`) matching the request's repo Purl, fetch with `Authorization: Bearer <TRAIL_GH_TOKEN>` if present, cache the response at `~/.principal/cache/files/<purl>/<sha>/<path>`. |
| 5 | `packages/principal-studio/src/bun/index.ts` (`getFileTree` RPC) | In remote mode, derive a synthetic tree from `markers[].sourcePath` instead of walking `repoRoot`. Buildings only need to exist for paths the trail references — no full tree required. |
| 6 | `packages/principal-studio/src/mainview/index.tsx` | Multi-repo `repository` resolution: when `payload.repos.length > 1`, the renderer needs to know which `repo` a given `readFile` call belongs to. Phase 3 #5 in the rollout doc covers the `(repo: Purl, path: string)` action contract. v1 single-repo can ship with the existing fallback we just put in. |
| 7 | `packages/principal-studio/src/bun/index.ts` (RPC schema) | Extend `readFile` params from `{ path }` to `{ path, repo?: Purl }` so multi-repo trails can route the right slice request to the right `repos[i]`. Backward-compatible: missing `repo` falls back to `repos[0]` / `authoredAt`. |
