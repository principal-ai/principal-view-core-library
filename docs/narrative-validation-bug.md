# Narrative Validation Bug Report

## Summary

The `narrative validate` command incorrectly reports that events referenced in a narrative file are "not defined in canvas", even when those events are correctly defined in the canvas file with matching names.

## Environment

- **Tool**: `@principal-ai/principal-view-cli`
- **Command**: `npx @principal-ai/principal-view-cli narrative validate <file>`
- **Date Reported**: 2026-01-25
- **Project**: web-ade

## Issue Description

When validating a narrative JSON file that references events from an OTEL canvas, the validator reports errors claiming the events don't exist in the canvas, despite:

1. The canvas file validating successfully
2. The event names matching exactly between canvas and narrative
...
2. Create a narrative JSON file referencing those events:
   ```json
   {
     "version": "1.0.0",
...
4. Validate narrative: `npx @principal-ai/principal-view-cli narrative validate .principal-views/auth-callback.narrative.json`
   - **Result**: ❌ Reports events not defined in canvas

## Actual Output

```
Validating: .principal-views/auth-callback.narrative.json

✗ Error: Narrative references event "auth.callback.complete" which is not defined in canvas
  Location: events
  Impact: This event will never highlight a canvas node and may never match
  Suggestion: Add event "auth.callback.complete" to a node in auth-callback.otel.canvas or remove it from the narrative

✗ Error: Narrative references event "auth.state.validated" which is not defined in canvas
  Location: events
  Impact: This event will never highlight a canvas node and may never match
  Suggestion: Add event "auth.state.validated" to a node in auth-callback.otel.canvas or remove it from the narrative

✗ Error: Narrative references event "auth.callback.started" which is not defined in canvas
  Location: events
  Impact: This event will never highlight a canvas node and may never match
  Suggestion: Add event "auth.callback.started" to a node in auth-callback.otel.canvas or remove it from the narrative

✗ Error: Narrative references event "auth.tokens.received" which is not defined in canvas
  Location: events
  Impact: This event will never highlight a canvas node and may never match
  Suggestion: Add event "auth.tokens.received" to a node in auth-callback.otel.canvas or remove it from the narrative

✗ Error: Narrative references event "auth.cookies.set" which is not defined in canvas
  Location: events
  Impact: This event will never highlight a canvas node and may never match
  Suggestion: Add event "auth.cookies.set" to a node in auth-callback.otel.canvas or remove it from the narrative

✗ Error: Narrative references event "auth.callback.error" which is not defined in canvas
  Location: events
  Impact: This event will never highlight a canvas node and may never match
  Suggestion: Add event "auth.callback.error" to a node in auth-callback.otel.canvas or remove it from the narrative
...
Events referenced in narrative file:
```bash
$ jq '.scenarios[].template.events | keys[]' .principal-views/auth-callback.narrative.json | sort -u
"auth.callback.complete"
"auth.callback.error"
"auth.callback.started"
"auth.cookies.set"
...
The narrative validation should:
...
3. Compare against events referenced in narrative scenarios
...
**Narrative file**: `/Users/griever/Developer/web-ade/web-ade/.principal-views/auth-callback.narrative.json`
...
- The issue only appears when validating narrative files
...
1. **Canvas loading issue**: The narrative validator might not be correctly loading or parsing the referenced canvas file
...
3. **Path resolution**: The canvas file path might not be resolving correctly relative to the narrative file
...
1. Add debug logging to narrative validator to show:
   - Which canvas file is being loaded
   - What events are extracted from the canvas
   - What events are being checked from the narrative
...
3. Verify the canvas file is being loaded from the correct path relative to the narrative file location
