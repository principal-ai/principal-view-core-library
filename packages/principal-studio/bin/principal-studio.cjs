#!/usr/bin/env node
/**
 * Bin shim for `@principal-ai/principal-studio`.
 *
...
if (platform !== 'darwin' || arch !== 'arm64') {
  process.stderr.write(
    `principal-studio: no prebuilt bundle for ${platform}-${arch} yet. Currently only darwin-arm64 is shipped.\n`,
  );
  process.exit(1);
}

const bundleRoot = join(__dirname, '..', 'bundles', 'macos-arm64', 'principal-studio.app');
const launcher = join(bundleRoot, 'Contents', 'MacOS', 'launcher');

if (!existsSync(launcher)) {
  process.stderr.write(
    `principal-studio: launcher missing at ${launcher}. The package may have been installed without its bundle.\n`,
  );
  process.exit(1);
}
...
child.on('error', (err) => {
  process.stderr.write(`principal-studio: failed to launch — ${err.message}\n`);
  process.exit(1);
});
