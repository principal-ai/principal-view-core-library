#!/usr/bin/env bun
/**
 * Stage the canary build into `bundles/<os>-<arch>/principal-studio.app/` so the
 ...
 * menu bar / app switcher shows `DISPLAY_NAME` instead of `principal-studio-canary`.
 */
...
const SRC_APP = join(pkgRoot, "build", "canary-macos-arm64", "principal-studio-canary.app");
const DEST_DIR = join(pkgRoot, "bundles", "macos-arm64");
const DEST_APP = join(DEST_DIR, "principal-studio.app");

/** Product name shown in the macOS menu bar, Dock, and app switcher. */
const DISPLAY_NAME = "Principal Studio";
