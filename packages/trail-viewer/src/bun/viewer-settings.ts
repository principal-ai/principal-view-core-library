 ...
 *
 * Single JSON file at `~/.principal/trail-viewer-settings.json`. Missing or
...
const STORE_PATH = join(homedir(), ".principal", "trail-viewer-settings.json");
...
export function saveViewerSettings(settings: ViewerSettings): ViewerSettings {
	...
	try {
		...
		console.error(
			`[trail-viewer] failed to write settings: ${(err as Error).message}`,
		);
	}
	return normalized;
}
